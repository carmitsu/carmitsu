...
export default function About({about}: Lang) {
  ...
  const [isWhyCarmitsuImage, setWhyCarmitsuImage] = useState(false);
      ...
      <div className="grid md:grid-cols-2 gap-4 max-md:grid-rows-2 container">
        <div className="space-y-6 md:my-8 lg:my-14 max-md:mb-3">
          ...
          <div className="space-y-2 grid lg:max-w-[40rem]">
            <div className="bg-default-100 p-2 rounded-lg space-y-1">
        ...
        <Skeleton className="rounded-xl relative" isLoaded={isWhyCarmitsuImage}>
          <Image src={whyUs} alt="ChooseUs Image" fill={true} className="object-center object-cover pointer-events-none"
                 onLoad={() => setWhyCarmitsuImage(true)}/>
        </Skeleton>
      </div>
    </section>
  );
